...
export default function AmbassadorPanel() {
  ...
  const activateWallet = async () => {
    ...
    try {
      ...
      if (axios.isAxiosError(error) && error.response?.status === 404) setMessage("This wallet is not on the approved Ambassador list.");
      else if ((error as Error).message?.toLowerCase().includes("rejected")) setMessage("Signature request was cancelled. No transaction was sent.");
...
function Leaderboard({ entries, isLoading, isRefreshing, updatedAt, onRefresh }: { entries: LeaderboardEntry[]; isLoading: boolean; isRefreshing: boolean; updatedAt: string | null; onRefresh: () => void }) {
  return <div className="overflow-hidden rounded-3xl border border-zinc-800 bg-zinc-900 shadow-xl"><div className="flex items-center justify-between gap-4 border-b border-zinc-800 px-5 py-4 sm:px-6"><div><h3 className="flex items-center gap-2 text-sm font-black uppercase tracking-wide text-zinc-100"><Trophy size={17} className="text-emerald-400" /> Season 1 Leaderboard</h3><p className="mt-1 text-xs text-zinc-500">Verified activity only{updatedAt ? ` · Updated ${new Date(updatedAt).toLocaleTimeString()}` : ""}</p></div><button type="button" onClick={onRefresh} disabled={isRefreshing} className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-zinc-700 text-zinc-400 transition-colors hover:border-emerald-500/40 hover:text-emerald-400 disabled:opacity-50" aria-label="Refresh leaderboard"><RefreshCw size={16} className={isRefreshing ? "animate-spin" : ""} /></button></div><div className="hidden overflow-x-auto md:block"><table className="w-full min-w-[620px] text-left text-sm"><thead className="bg-zinc-950/50 text-[10px] font-mono uppercase tracking-wider text-zinc-500"><tr><th className="px-6 py-3 font-medium">Rank</th><th className="px-3 py-3 font-medium">Creator</th><th className="px-3 py-3 text-right font-medium">Points</th><th className="px-3 py-3 text-right font-medium">Referrals</th><th className="px-3 py-3 text-right font-medium">Clean Dust</th><th className="px-6 py-3 text-right font-medium">Bridge Volume</th></tr></thead><tbody className="divide-y divide-zinc-800/80">{entries.map((entry) => <tr key={entry.rank} className="text-zinc-300"><td className="px-6 py-4 font-mono text-zinc-500">#{entry.rank} {entry.isTop50 && <Trophy className="ml-1 inline text-emerald-400" size={14} aria-label="Top 50" />}</td><td className="px-3 py-4"><p className="font-bold text-zinc-100">{entry.creator}</p>{entry.xHandle && <p className="mt-0.5 text-xs text-zinc-500">@{entry.xHandle}</p>}</td><td className="px-3 py-4 text-right font-mono font-bold text-emerald-400">{formatNumber(entry.points)}</td><td className="px-3 py-4 text-right font-mono">{formatNumber(entry.referrals)}</td><td className="px-3 py-4 text-right font-mono">{formatNumber(entry.coinsSwept)}</td><td className="px-6 py-4 text-right font-mono">{formatUsd(entry.bridgeVolumeUsd)}</td></tr>)}</tbody></table></div><div className="divide-y divide-zinc-800 md:hidden">{entries.map((entry) => <article key={entry.rank} className="p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-mono text-xs text-zinc-500">#{entry.rank} {entry.isTop50 && <span className="text-emerald-400">· Top 50</span>}</p><p className="mt-1 font-bold text-zinc-100">{entry.creator}</p>{entry.xHandle && <p className="text-xs text-zinc-500">@{entry.xHandle}</p>}</div><p className="font-mono font-bold text-emerald-400">{formatNumber(entry.points)} pts</p></div><dl className="mt-4 grid grid-cols-3 gap-2 text-center text-xs"><Metric label="Refs" value={formatNumber(entry.referrals)} /><Metric label="Clean" value={formatNumber(entry.coinsSwept)} /><Metric label="Bridge" value={formatUsd(entry.bridgeVolumeUsd)} /></dl></article>)}</div>{!isLoading && entries.length === 0 && <p className="px-6 py-10 text-center text-sm text-zinc-500">The Season 1 leaderboard will appear as approved ambassadors begin qualifying activity.</p>}{isLoading && <p className="px-6 py-10 text-center text-sm text-zinc-500">Loading verified leaderboard…</p>}</div>;
}
