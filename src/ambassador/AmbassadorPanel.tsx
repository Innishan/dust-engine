...
interface AmbassadorProfile extends Omit<LeaderboardEntry, "walletAddress" | "isTop50"> {
  displayName: string;
  referralCode: string;
  xUsername?: string;
  xVerified: boolean;
...
const leaderboardIdentity = (entry: LeaderboardEntry) => entry.xUsername ? `@${entry.xUsername}` : `${entry.walletAddress.slice(0, 8)}•••${entry.walletAddress.slice(-4)}`;
